...
public:
    ...
    void buildPipeline() {
        ...
        if (schedulerEnabled || evoEnabled) {
            if (useERL) {
                ...
                // [BUILD FIX / S1 WIRING] Forward the actual AlgorithmConfig
                // identity into the ERL controller. EvolutionarySelector cannot
                // infer the active workload from its Scheduler/EvolutionarySelector
                // sub-config alone. Capability is derived here from the same
                // algorithmType used to construct AlgorithmModule.
                const std::string activeAlgorithm =
                    jvalue<std::string>(jAlg, "algorithmType", "Invert");
                const bool continuousGpuSplit =
                    (activeAlgorithm == "HeterogeneousGaussianBlur");

                jBrain["active_algorithm"] = activeAlgorithm;
                jBrain["workload_id"] = hrl::workloadIdFor(activeAlgorithm);
                jBrain["gpu_split_applicable"] = continuousGpuSplit;

                spdlog::info(
                    "[ConfigManager] ERL workload='{}' continuous_gpu_split={}",
                    activeAlgorithm, continuousGpuSplit ? "YES" : "NO");
    ...
    // =====================================================================
    // Runtime algorithm-only workload transition.
    // =====================================================================
    // ConfigManager itself is NOT rebuilt. All module objects remain owned by
    // the same modules_ vector. EvolutionarySelector pauses only GA evolution
    // long enough to create a clean experimental boundary.
    bool switchAlgorithmWorkload(const hrl::WorkloadProfile& target)
    {
        if (!algorithmModule_) {
            spdlog::error("[WORKLOAD] AlgorithmModule unavailable");
            return false;
        }
        if (target.label.empty() || target.workloadId == 0) {
            spdlog::error("[WORKLOAD] Invalid target profile: label='{}' id={}",
                          target.label, target.workloadId);
            return false;
        }

        const std::string previousAlgorithm = algorithmModule_->currentWorkload();

        // beginWorkloadTransition() is a GA barrier, NOT a controller restart:
        // an in-flight generation may finish, then no new generation starts
        // until the algorithm worker and workload context agree again.
        if (evolutionarySelector_) {
            // The selector returns the generation while its GA barrier is held.
            // Do not re-sample outside the barrier: ERL may resume immediately
            // after commit, which would make the evidence timing-dependent.
            lastTransitionGenerationBefore_ =
                evolutionarySelector_->beginWorkloadTransition(target);
            lastTransitionGenerationAfter_ = lastTransitionGenerationBefore_;
        } else {
            lastTransitionGenerationBefore_ = 0u;
            lastTransitionGenerationAfter_ = 0u;
        }

        bool algorithmSwitched = false;
        try {
            algorithmSwitched = algorithmModule_->switchWorkload(target);
        } catch (const std::exception& e) {
            spdlog::error("[WORKLOAD] Algorithm-only transition threw: {}", e.what());
        } catch (...) {
            spdlog::error("[WORKLOAD] Algorithm-only transition threw an unknown exception");
        }

        if (!algorithmSwitched) {
            if (evolutionarySelector_) evolutionarySelector_->cancelWorkloadTransition();
            return false;
        }

        try {
            if (evolutionarySelector_) {
                // commitWorkloadTransition() captures this value while gaMutex_
                // is still held and before evolution is resumed.
                lastTransitionGenerationAfter_ =
                    evolutionarySelector_->commitWorkloadTransition(target);
            } else {
                lastTransitionGenerationAfter_ = 0u;
            }
        } catch (const std::exception& e) {
            // Extremely defensive rollback. AlgorithmModule's hot swap is itself
            // transactional, but if the ERL context commit unexpectedly fails,
            // restore the previous algorithm so controller/workload never diverge.
            spdlog::critical(
                "[WORKLOAD] ERL context commit failed after algorithm swap: {}. Rolling back to {}",
                e.what(), previousAlgorithm);
            try {
                algorithmModule_->switchWorkload(previousAlgorithm);
            } catch (...) {}
            if (evolutionarySelector_) evolutionarySelector_->cancelWorkloadTransition();
            return false;
        }

        // Keep ConfigManager's audit copy coherent. This does not rebuild any module.
        config_["AlgorithmConfig"]["algorithmType"] = target.label;
        config_["AlgorithmConfig"]["useGPU"] = target.gpuCapable;

        spdlog::info(
            "[ConfigManager] Workload switch committed: id={} {} | "
            "SAME ConfigManager / Scheduler / GA / AWM / telemetry pipeline",
            target.workloadId, target.label);
        return true;
    }

    bool switchAlgorithmWorkload(const std::string& workload)
    {
        // Compatibility path. The dynamic mission should pass a full profile so
        // GPU capability and the workload-specific surrogate ceiling are explicit.
        return switchAlgorithmWorkload(hrl::makeWorkloadProfile(workload, true, 60.0));
    }

    // Mission-only warmup gate: all hardware/telemetry modules run normally,
    // but GA evolution is held at generation 0 until the measurement clock starts.
    void pauseERLEvolutionForMissionWarmup()
    {
        if (evolutionarySelector_) {
            evolutionarySelector_->pauseEvolutionForMissionWarmup();
        }
    }

    size_t resumeERLEvolutionAfterMissionWarmup()
    {
        return evolutionarySelector_
            ? evolutionarySelector_->resumeEvolutionAfterMissionWarmup()
            : 0u;
    }
...
public:
    ...
    // Dynamic-adaptation evidence accessors. These are read-only views into the
    // persistent ERL instance and are used by the mission telemetry CSV.
    size_t getERLGeneration() const {
        return evolutionarySelector_ ? evolutionarySelector_->getCurrentGeneration() : 0u;
    }

    std::string getERLRegimeName() const {
        return evolutionarySelector_
            ? evolutionarySelector_->getCurrentRegimeName()
            : std::string("NO_ERL");
    }

    std::string getActiveERLWorkload() const {
        return evolutionarySelector_
            ? evolutionarySelector_->getCurrentWorkload()
            : (algorithmModule_ ? algorithmModule_->currentWorkload() : std::string("UNKNOWN"));
    }

    uint32_t getActiveERLWorkloadId() const {
        return evolutionarySelector_ ? evolutionarySelector_->getCurrentWorkloadId() : 0U;
    }

    double getLastAlgorithmSwitchDowntimeMs() const {
        return algorithmModule_ ? algorithmModule_->lastSwitchDowntimeMs() : 0.0;
    }


    size_t getLastAlgorithmInputDrainedFrames() const {
        return algorithmModule_
            ? algorithmModule_->lastSwitchInputDrainedFrames()
            : 0U;
    }

    size_t getLastAlgorithmOutputDrainedFrames() const {
        return algorithmModule_
            ? algorithmModule_->lastSwitchOutputDrainedFrames()
            : 0U;
    }

    bool algorithmStarved() const {
        return algorithmModule_ ? algorithmModule_->algorithmStarved() : false;
    }

    size_t getLastTransitionGenerationBefore() const {
        return lastTransitionGenerationBefore_;
    }

    size_t getLastTransitionGenerationAfter() const {
        return lastTransitionGenerationAfter_;
    }

    uint64_t getWorkloadEpoch() const {
        return evolutionarySelector_ ? evolutionarySelector_->getWorkloadEpoch() : 0ULL;
    }

    uint64_t getCurrentWorkloadSurrogateObservations() const {
        return evolutionarySelector_
            ? evolutionarySelector_->getCurrentWorkloadSurrogateObservations()
            : 0ULL;
    }

    bool isWorkloadTransitionInProgress() const {
        return evolutionarySelector_
            ? evolutionarySelector_->isWorkloadTransitionInProgress()
            : false;
    }

    std::string exportERLSurrogateCSV() const {
        return evolutionarySelector_
            ? evolutionarySelector_->exportSurrogateCSV()
            : std::string("workload_id,workload\n");
    }

    std::string exportERLWeightHistoryCSV() const {
        return evolutionarySelector_
            ? evolutionarySelector_->exportWeightHistoryCSV()
            : std::string("adaptive_weights_disabled\n");
    }
...
private:
    ...
    // Exact generation sampled inside the begin/commit workload barrier.
    size_t lastTransitionGenerationBefore_ = 0u;
    size_t lastTransitionGenerationAfter_  = 0u;
};
