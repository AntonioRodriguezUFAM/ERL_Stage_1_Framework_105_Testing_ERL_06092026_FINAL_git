...
// Find the lynsyn_prerelease function and add the highlighted checks.
void lynsyn_prerelease(void) {
   if (lynsynHandle) {
    ...
    lynsynHandle = NULL; // Good practice to nullify after close
  }

   if (devs) {
    libusb_free_device_list(devs, 1);
    devs = NULL; // Good practice to nullify after free
  }
  //  if (lynsynHandle) {
  //   libusb_close(lynsynHandle);
  //   lynsynHandle = NULL; // Good practice
  // }
  if (usbContext) {
    libusb_exit(usbContext);
    usbContext = NULL; // Good practice
  }
