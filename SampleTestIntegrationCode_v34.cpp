...
static std::vector<BenchmarkCase> build_benchmark_matrix()
{
    // Static configurations used for the post-hoc Oracle comparison.
    static const std::vector<ControlMode> baselineModes = {
        ControlMode::CPU_STATIC,
        ControlMode::GPU_STATIC,
        ControlMode::BALANCED
    };
    ...
    std::vector<BenchmarkCase> matrix;
    matrix.reserve(4 * baselineModes.size());

    for (const auto& algo : kAlgorithms) {

        // Canonical dynamic-mission workloads only.
        if (algo.label != "HistogramEqualization" &&
            algo.label != "SobelEdge" &&
            algo.label != "HeterogeneousGaussianBlur" &&
            algo.label != "MedianFilter") {
            continue;
        }

        for (const auto mode : baselineModes) {
            BenchmarkCase bc;
            bc.algo = algo;
            bc.mode = mode;
            bc.suite = "StaticOracle_" + algo.label;
            bc.testLabel =
                algo.label + "_" + control_mode_to_string(mode);
...
static void pin_static_controls(
    const BenchmarkCase& bc,
    const std::shared_ptr<hrl::RuntimeControls>& rt)
{
    if (!rt) {
    ...
    switch (bc.mode) {
        ...
        case ControlMode::GPU_STATIC:
            ...
            rt->gpu_workload_split.store(
                bc.algo.gpuCapable ? 1.0 : 0.0,
                std::memory_order_relaxed);
